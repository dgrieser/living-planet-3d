...
- "Conditions in the solar system" (`neighbourhoodState()`, rendered as four prose paragraphs) scales published present-day anchors
  ...
  the odds are quoted as the chance of at least one event in any 100 Myr, 1 − exp(−100/T). A verdict
  sentence rates how close conditions would be to today's from the density ratio (≤ 1.33 close, ≤ 2.5 shifted,
  else different), and the birthplace paragraph switches on metallicity (≥ 1.1 rich, ≤ 0.9 poor): metal-rich
